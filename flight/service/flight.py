...
class IFlightAdapter(ABC):
    @abstractmethod
    def search_flights(self, outbound_flight: Flight, return_flight: Flight) -> list:
        pass
...
class MockAirlineIncIntegration(IFlightAdapter):
    def __init__(
        self,
    ...
    def search_flights(
        ...
        flight_combinations = sorted(
            flight_combinations, key=lambda k: k["price"]["total"]
        )
    ...
    def mount_flight_combination(
        ...
        for outbound_flight in outbound_flights.options:
            for flight_back in return_flights.options:
                total_price = self.build_price(
                    fare=outbound_flight.price.fare + flight_back.price.fare,
                    fees=0,
                    total=0,
                )
                ...
                flight_combinations.append(flight_combination.to_dict())
